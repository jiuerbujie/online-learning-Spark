...
// 模型为 1/(1+exp(-w*x+w_offset))
object doStreamBatchLogisticRegression {
    ...
    def main(args: Array[String]) {
        ...
        """
          ...
          |  --step 0.1 --numDataInBatch 40 --dimenOfData 9\
          |  parameters.txt
        """.stripMargin)
	    }
        ...
        if(f.length() >= 10+dimenOfData)
        	{
        		...
        		{//Run Logistic Regression Here
        			...
        			numDataInArray = 0
        			//对此batch运行LR，初始参数为上一个batch的结果
        			w_stream = runLR(dataPointArray, numIteration, stepsize,w_stream)
...
def generatePoint(i: Int, data:Array[Double]) = {
  ...
  def runLR(dataPoints:List[DataPoint], iteration: Int,step:Double, w_int:DenseVector[Double]):DenseVector[Double] ={
    var w = w_int
